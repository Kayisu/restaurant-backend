...
SELECT 
  ...
  SUM(t.capacity) FILTER (WHERE t.is_occupied = false AND t.is_reserved = false) as available_capacity,

  -- Section status
  ROUND(
    (COUNT(*) FILTER (WHERE t.is_occupied = true) + COUNT(*) FILTER (WHERE t.is_reserved = true)) * 100.0 / COUNT(*), 
    1
  ) as occupancy_rate,
  ...
  ROUND(
    AVG(dt.occupied_duration_minutes) FILTER (WHERE t.is_occupied = true), 
    1
...
SELECT 
  ...
  dt.reservation_id,
  dt.reserved_party_size,
  dt.reservation_date,
  dt.reservation_time,
  dt.reservation_status,
  dt.reserved_customer_name,
  dt.reserved_customer_phone,
